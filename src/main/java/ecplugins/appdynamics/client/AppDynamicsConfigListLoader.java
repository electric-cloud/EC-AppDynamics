...
// AppDynamicsConfigListLoader.java --
//
// AppDynamicsConfigListLoader.java is part of ElectricCommander.
//
// Copyright (c) 2005-2011 Electric Cloud, Inc.
// All rights reserved.
//

package ecplugins.appdynamics.client;
...
public class AppDynamicsConfigListLoader
    extends Loader
{

    //~ Instance fields --------------------------------------------------------

    private final AppDynamicsConfigList   m_configList;
    private final CgiRequestProxy m_cgiRequestProxy;
    private String                m_editorName;

    //~ Constructors -----------------------------------------------------------

    public AppDynamicsConfigListLoader(
            AppDynamicsConfigList   configList,
            Component       component,
            ...
            ChainedCallback callback)
    {
        ...
        m_cgiRequestProxy = new CgiRequestProxy(getPluginName(), "appdynamics.cgi");
    }
    ...
    private void loadConfigs(Map<String, String> cgiParams)
    {
        ...
        try {
                    ...
                    new RequestCallback() {
                                ...
                                Throwable exception)
                        {
                            ((HasErrorPanel) m_component).addErrorMessage(
                                "Error loading AppDynamics configuration list: ",
                                exception);
                        }
            ...
            if (m_component instanceof HasErrorPanel) {
                ((HasErrorPanel) m_component).addErrorMessage(
                    "Error loading AppDynamics configuration list: ", e);
            }
            else {
                m_component.getLog()
                           .error(e);
    ...
    private void loadEditors()
    {
        ...
        request.setPropertyName("/plugins/EC-AppDynamics/project/forms/"
                + m_editorName);
        request.setExpand(false);
        request.setCallback(new EditorLoaderCallback("appdynamicscfg"));
        ...
        implements PropertyCallback
    {
        ...
        @Override public void handleResponse(Property response)
        {
            ...
            String errorMsg = "Editor '" + m_editorName
                    + "' not found for AppDynamics plugin '" + m_configPlugin + "'";
