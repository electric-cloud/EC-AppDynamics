...
sub decode_json_safe {
    ...
    my $object;
    eval {
        $object = decode_json($json);
        1;
    } or do {
        $object = {};
        $self->{_last_decode_json_error} = $@;
    };
...
sub encode_json_safe {
    ...
    my $json;
    eval {
        $json = encode_json($object);
        1;
    } or do {
        $json = '';
        $self->{_last_encode_json_error} = $@;
    };
    return $json;
}
...
sub rest_client {
    ...
    return EC::REST::Client->new(@params);
}
...
sub new {
    ...
    $params{url} =~ s|/$||;
    $self->{_data}->{url} = $params{url};
    if ($params{auth_basic}) {
        my $ab = $params{auth_basic};
        unless ($ab->{user} && $ab->{password}) {
            croak "Missing required data for basic auth";
        }
        $self->{_auth_basic} = $ab;
    }
    if ($params{accept}) {
        $self->{_data}->{accept} = $params{accept};
    }
...
sub call {
    my ($self, $method, $url, $data, $content) = @_;

    my $request_url = $self->{_data}->{url} . $url;
    my $req = HTTP::Request->new($method => $request_url);
    if (ref $data eq 'HASH') {
        for my $key (keys %$data) {
            $req->header($key => $data->{$key});
        }
    }
    elsif (! ref $data) {
        $req->header('Content-Type' =>  $data);
    }
    else {
        croak "Data parameter should be a content type or hashref of headers";
    }
    if ($self->{_data}->{accept}) {
        $req->header('Accept' => $self->{_data}->{accept});
    }
    if ($self->{_auth_basic}) {
        $req->authorization_basic($self->{_auth_basic}->{user}, $self->{_auth_basic}->{password});
    }
    $req->content($content);
